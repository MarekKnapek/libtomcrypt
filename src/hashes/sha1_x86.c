...
#include <emmintrin.h> /* SSE2 _mm_load_si128 _mm_store_si128 _mm_set_epi32 _mm_set_epi64x _mm_setzero_si128 _mm_xor_si128 _mm_add_epi32 _mm_shuffle_epi32 */
...
#endif
{
    ...
    LTC_ARGCHK(((uintptr_t)(buf)) % 16 == 0);
    LTC_ARGCHK(sizeof(int) == 4);
    /*LTC_ARGCHK(cpuid_has_sse2());
    LTC_ARGCHK(cpuid_has_ssse3());
    LTC_ARGCHK(cpuid_has_sse41());
    LTC_ARGCHK(cpuid_has_sha());*/
...
int  sha1_x86_test(void)
{
  ...
  for (i = 0; i < (int)(sizeof(tests) / sizeof(tests[0]));  i++) {
      sha1_x86_init(&md);
      sha1_x86_process(&md, (unsigned char*)tests[i].msg, (unsigned long)XSTRLEN(tests[i].msg));
      sha1_x86_done(&md, tmp);
      if (ltc_compare_testvector(tmp, sizeof(tmp), tests[i].hash, sizeof(tests[i].hash), "SHA1-x86", i)) {
