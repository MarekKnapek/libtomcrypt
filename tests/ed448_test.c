...
static int s_rfc_8032_5_2_6_pure_test(void)
{
   ...
   for (n = 0; n < sizeof(vectors)/sizeof(vectors[0]); ++n) {
      slen = sizeof(sec);
      DO(base16_decode(vectors[n].secret_key, (unsigned long)XSTRLEN(vectors[n].secret_key), sec, &slen));
      plen = sizeof(pub);
      DO(base16_decode(vectors[n].public_key, (unsigned long)XSTRLEN(vectors[n].public_key), pub, &plen));
      if (XSTRLEN(vectors[n].message) > 0) {
         mlen = sizeof(msg);
         DO(base16_decode(vectors[n].message, (unsigned long)XSTRLEN(vectors[n].message), msg, &mlen));
      } else {
         mlen = 0;
      }
      siglen = sizeof(sig);
      DO(base16_decode(vectors[n].signature, (unsigned long)XSTRLEN(vectors[n].signature), sig, &siglen));
...
static int s_rfc_8032_5_2_6_ctx_test(void)
{
   ...
   for (n = 0; n < sizeof(vectors)/sizeof(vectors[0]); ++n) {
      ...
      DO(base16_decode(vectors[n].signature, (unsigned long)XSTRLEN(vectors[n].signature), sig, &siglen));
      ctxlen = sizeof(ctx);
      DO(base16_decode(vectors[n].context, (unsigned long)XSTRLEN(vectors[n].context), ctx, &ctxlen));
